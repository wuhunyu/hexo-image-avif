...
const { resolveTarget } = require('./pathing');
const { assertNoCollisions } = require('./collisions');
...
async function discover(sourceDir) {
  ...
  for (const markdownPath of files) {
    ...
    const references = findImageReferences(content).map(reference => {
...
async function processImages(hexo, deps = {}) {
  ...
  let discovery;
  try {
    discovery = await discover(sourceDir);
  } catch (error) {
  ...
  hexo.log.info(
    `[hexo-image-avif] Done: ${summary.found} references, ${summary.converted} converted, ${summary.reused} reused, ${summary.failed} failed, ${summary.updatedFiles} files updated`,
  );
  return summary;
}
